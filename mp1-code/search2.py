...
class State():
    def __init__(self, pos, explored):
        self.pos = pos
        self.explored = explored

    def __hash__(self):
        self.hash = hash((tuple(self.explored), self.pos))
        return self.hash
...
def bfs(maze):
    ...
    visited = set()
    # explored = set()
    initial = State(start, set())
    queue = deque([(initial, obj, path)])
    while queue:
        state, obj, path = queue.popleft()
        if state in visited:
            continue
        cur = state.pos
        obj_left = obj.copy()
        cur_explored = state.explored
        ...
        if cur in obj_left:
            ...
            if len(obj_left) == 0:
                return path, num_states_explored
        # t_obj_left = tuple(obj_left)
        # t_explored = tuple(cur_explored)
        state = State(cur, cur_explored)
        visited.add(state)
        ...
        for n in nei:
            ...
            # temp_explored.add(n)
            n_state = State(n, cur_explored)
            if n_state in visited:
                # print("n in visited")
                continue
            queue.append((n_state, obj_left, path + [n]))
    return [], 0
